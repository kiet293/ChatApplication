			...
			if (loginResult.equals("login success")) {
				...
				receiveAndProcessThread = new Thread(() -> {
					try {
						while (true) {
							...
							case "user quit": {
								...
								for (Room room : allRooms) {
									if (room.users.contains(whoQuit)) {
										Main.mainScreen.addNewMessage(room.id, "notify", whoQuit, "Quit the app");
							...
							case "new room": {
								...
								Main.mainScreen.addNewMessage(newRoom.id, "notify", whoCreate,
										type.equals("group") ? "Group created" : "Chat opened");
						...
						JOptionPane.showMessageDialog(Main.mainScreen, "The server is closed, the application will exit", "Notify",
